...
router.post('/fetch-problems', async (req, res) => {
  ...
  try {
    ...
    const problems = data.result.problems.filter(problem => problem.tags.includes(topic) && problem.rating >= minRating && problem.rating <= maxRating);
    if (problems.length < numProblems) {
      return res.status(400).json({
        error: `Not enough problems available for the given criteria. Found only ${problems.length} problems.`
      });
    }
    const mashup = [];
